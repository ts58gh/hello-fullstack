...
试玩脚本会默认访问公共后端 **`https://hello-fullstack-py.onrender.com`**。页面可勾选「其他座位自动出牌」，由脚本代出首张合法牌，便于单人试完整局。若打不开或你从 fork 自部署后端，须在 Render **环境变量** 中设置 **`CORS_ALLOW_ORIGINS`**（允许多个，逗号分隔），包含你的前端源站，通常为 **`https://<用户名>.github.io`**。
